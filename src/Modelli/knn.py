...
2. standardizzazione (fondamentale per KNN basato su distanze);
3. GridSearchCV per trovare i migliori iperparametri;
4. classificatore KNeighborsClassifier.

Gli iperparametri tuned sono:
- n_neighbors: numero di vicini considerati
- weights: 'uniform' o 'distance' (peso inverso alla distanza)
- metric: distanza Euclidea, Manhattan o Minkowski

Le geo feature aggregate sono attive di default per coerenza con SVM.
"""
...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "outputs" / "modelli" / "knn"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Addestra e valuta un KNN multiclasse con hyperparameter tuning."
    )
    parser.add_argument(
        "--dataset",
        type=Path,
...
def run_hyperparameter_tuning(
    ...
    """Esegue il grid search sui parametri KNN usando il modulo Hyperparameter_Tuning.
    ...
    """
    # Ottieni la configurazione KNN dal modulo di hyperparameter tuning
